...
random.seed(1)
x_train, y_train, x_test, y_test = u.return_diabetes_processed_data()


# write out train data
diabetes_train = r'diabetes_data_train.csv'
df_train = pd.DataFrame(data=x_train)
df_train[10] = y_train 
df_train.to_csv(diabetes_train, index =False, header =False)

# write out test data
diabetes_test = r'diabetes_data_test.csv'
df_test = pd.DataFrame(data=x_test)
df_test[10] = y_test
df_test.to_csv(diabetes_test, index =False, header =False)


logz_file_name = r'logz_diabetes.csv'
mse_train_file_name = r'mse_train_diabetes.csv'
mse_test_file_name = r'mse_test_diabetes.csv'
...
alpha = pd.read_excel("alpha_diabetes_mean.xlsx").values
beta = pd.read_excel("beta_diabetes_mean.xlsx").values
...
data_logz = pd.read_csv("logz_diabetes.csv") 
data_mse_train = pd.read_csv("mse_train_diabetes.csv") 
data_mse_test = pd.read_csv("mse_test_diabetes.csv") 

logz_train_avarage = data_logz.mean()
mse_train_avarage = data_mse_train.mean()
mse_test_avarage = data_mse_test.mean()

with open(r'logz_diabetes_mean.csv', 'a') as f:
    writer = csv.writer(f)
    writer.writerow(logz_train_avarage)
    
with open(r'mse_train_diabetes_mean.csv', 'a') as f:
    writer = csv.writer(f)
    writer.writerow(mse_train_avarage)

with open(r'mse_test_diabetes_mean.csv', 'a') as f:
    writer = csv.writer(f)
...
plt.legend(loc=4)
plt.show()
fig.savefig('results/diabetes_log_evidence.png')
...
plt.legend(loc=1)
plt.show()
fig.savefig('results/diabetes_mse_train.png')
...
plt.legend(loc=1)
plt.show()
fig.savefig('results/diabetes_mse_test.png')
